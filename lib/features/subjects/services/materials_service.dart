...
import 'package:test/core/constants/app_url.dart';
...
import 'package:test/features/subjects/domain/entities/materials_entity.dart'
    as materials;
import 'package:test/features/subjects/services/material_types.dart' as mt;
...
class MaterialsService {
  ...
  materials.MaterialUrlType getMaterialTypeFromExtension(String extension) {
    final materialType = mt.MaterialTypeUtils.getTypeFromExtension(extension);
    switch (materialType) {
      case mt.MaterialType.pdf:
        return materials.MaterialUrlType.pdf;
      case mt.MaterialType.image:
        return materials.MaterialUrlType.image;
      case mt.MaterialType.document:
        return materials.MaterialUrlType.document;
      case mt.MaterialType.excel:
        return materials.MaterialUrlType.excel;
      case mt.MaterialType.powerpoint:
        return materials.MaterialUrlType.powerpoint;
      case mt.MaterialType.link:
        return materials.MaterialUrlType.link;
      default:
        return materials.MaterialUrlType.other;
    }
  }

  /// Check if material can be viewed in-app
  bool canViewInApp(materials.MaterialUrlType type) {
    final materialType = _getMaterialTypeFromUrlType(type);
    return materialType.canViewInApp;
  }

  /// Check if material can be downloaded
  bool canDownload(materials.MaterialUrlType type) {
    final materialType = _getMaterialTypeFromUrlType(type);
    return materialType.canDownload;
  }

  /// Convert MaterialType to MaterialType
  mt.MaterialType _getMaterialTypeFromUrlType(materials.MaterialUrlType type) {
    switch (type) {
      case materials.MaterialUrlType.pdf:
        return mt.MaterialType.pdf;
      case materials.MaterialUrlType.image:
        return mt.MaterialType.image;
      case materials.MaterialUrlType.document:
        return mt.MaterialType.document;
      case materials.MaterialUrlType.excel:
        return mt.MaterialType.excel;
      case materials.MaterialUrlType.powerpoint:
        return mt.MaterialType.powerpoint;
      case materials.MaterialUrlType.word:
        return mt.MaterialType.document;
      case materials.MaterialUrlType.link:
        return mt.MaterialType.link;
      case materials.MaterialUrlType.other:
    ...
    try {
      ...
      try {
        await _downloadDio.download(
          ...
          options: Options(
            headers: {
              'User-Agent': 'UniSphere-Student/1.0',
            },
          ),
        );
      } catch (downloadError) {
  ...
  IconData getMaterialIcon(materials.MaterialUrlType type) {
    final materialType = _getMaterialTypeFromUrlType(type);
    return materialType.icon;
  }

  /// Get material icon color
  Color getMaterialIconColor(materials.MaterialUrlType type) {
    final materialType = _getMaterialTypeFromUrlType(type);
    return materialType.color;
  }

  /// Get action button icon
  IconData getActionIcon(materials.MaterialUrlType type) {
    final materialType = _getMaterialTypeFromUrlType(type);
  ...
  String getActionText(materials.MaterialUrlType type) {
    final materialType = _getMaterialTypeFromUrlType(type);
